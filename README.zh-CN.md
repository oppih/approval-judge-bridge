...
还剩两处瓶颈，都与"让这道门持续可用"有关（fail-closed 本身已有测试覆盖）。细则与验收判据见 [TODO.md](TODO.md)。

- **判定器 failover。** fail-closed 意味着判定器不可达时一律转人工，上游一次故障就把这门退化成"每条命令都弹确认"——正是本项目要消灭的症状。2026-09-21 实测：Jev 过载期间，同一批 17 条命令有 16 条没拿到判定就转人工（15 × `http_529`、1 × `TimeoutError`）。切换必须只发生在传输层失败（绝不对已判定的结果重新判定），每个判定器带自己的阈值，fallback 自身要能通过校准用例集，并在 `/healthz` 上显示是哪个判定器作答。
- **校准回归。** 17 条校准用例是手工调出来的，改措辞就能让同一条命令在 `approve` / `deny` 之间翻面；真实样本只有决策日志里的 37 条，且日志只保留一代。把这些命令固化为可重放的 fixture，让 rubric 改动无法静默翻转某条命令，并增加 "rubric diff" 模式在发布前打印翻面清单。
